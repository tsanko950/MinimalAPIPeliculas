...
namespace MinimalAPIPeliculas.Repositorios
{
    public class RepositorioActores : IRepositorioActores
    {
        ...
        public async Task<List<Actor>> ObtenerTodos(PaginacionDTO paginacionDTO)
        {
            ...
            return await context.Actores.OrderBy(a => a.Nombre).Paginar(paginacionDTO).ToListAsync();
        }
        ...
        public async Task<Actor?> ObtenerPorId(int id)
        {
            ...
            // AsNoTracking - para que no le de seguimiento al actor que retorne este metodo, si posteriormente se hace un SaveChanges este no provocara cambios
            // en la BD ya que tenemos el AsNoTracking
        }

        public  async Task<List<Actor>> ObtenerPorNombreActor(string nombre)
        {
